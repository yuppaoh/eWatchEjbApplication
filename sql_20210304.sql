...
CREATE TABLE `ewatch_data`.`customers` (
  ...
  `CustomerName` VARCHAR(45) NOT NULL,            -- sua lai la NULL cho phu hop voi form register
  `CustomerAddress` VARCHAR(45) NULL,
  `PhoneNo` VARCHAR(18) NULL,
  `CustomerStatus` VARCHAR(10) NULL DEFAULT 'Enable',
  PRIMARY KEY (`CustomerId`));

ALTER TABLE `ewatch_data`.`customers` 
CHANGE COLUMN `CustomerName` `CustomerName` VARCHAR(45) CHARACTER SET 'utf8' COLLATE 'utf8_unicode_ci' NULL ;
  
INSERT INTO `ewatch_data`.`customers` (`CustomerId`, `CustomerPassword`, `CustomerName`, `CustomerAddress`, `PhoneNo`, `CustomerStatus`) VALUES ('cus001', 'abc12345', 'Tan Hung', '01 Ly Tu Trong', '0901040714', 'Enable');  
